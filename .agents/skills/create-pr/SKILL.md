...
If the session is inside a Codex worktree (the working directory is under `.Codex/worktrees/`), the PR workflow is the same — commit, push, and create the PR from the worktree branch. But there are a few things to keep in mind:
...
- Remind the user they can exit the worktree with `exit` (or by asking Codex to exit). Use `action: "keep"` if they might want to iterate on the PR, or `action: "remove"` if the work is done.
